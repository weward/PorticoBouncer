...
const form = useForm({
    ...
    abilities: props.checkedAbilities,
});
...
const getAbilities = () => {
    return props.role.abilities.map(({ id }) => id)
}

// send to CheckBoxList
provide('propAbilities', props.abilities)
provide('propCheckedAbilities', getAbilities())
provide('updateFormAbilities', updateFormAbilities)

const submit = async (e) => {
    layoutData.toast = await toast('info', 'Updating...')

    await form.put(route('roles.update', props.role.id), {
...
<template>
    <AdminLayout :data="layoutData">
        ...
        <AppHead 
            :title="`Update User Role - ${props.role.title}`"
            :meta="{ 'description': 'Portico is an advance PHP Laravel login and user management system' }" 
        />
        ...
        <template #header>
            ...
            <h2 class="tw-font-bold tw-text-base tw-text-gray-800 tw-leading-tight tw-w-full ">
                {{ `Update User Role - ${props.role.title}` }}

                <PrimaryButton :disabled="!form.abilities?.length" @click="($event) => submit()"
                    class="tw-float-right tw-invisible md:tw-visible">
                    <VIcon icon="mdi-floppy-disk"></VIcon>
                    <span class="tw-text-xs">&nbsp;Update</span>
                </PrimaryButton>

                <LinkButton type="text" class="tw-hidden md:tw-inline tw-float-right" :href="route('roles.index')">
                        ...
                        <VCard class="">
                            ...
                            <VCardText>
                                <VForm @submit.prevent="submit" class="tw-p-1 md:tw-p-6">
                                    <VRow class="tw-pb-12">
                                        <VCol>
                                            <VTextField v-model="form.title" label="Role Name" type="text" required />
                                    ...
                                    <div class="tw-text-center tw-pb-6">
                                        ...
                                        <PrimaryButton :disabled="!form.abilities?.length" @click="($event) => submit()"
                                            class="tw-visible md:tw-invisible tw-mb-6 " block>
                                            <VIcon icon="mdi-pencil"></VIcon>
                                            Update
                                        </PrimaryButton>

                                        <LinkButton :href="route('roles.index')" type="text" class="tw-block md:tw-hidden ">
                                            <VIcon icon="mdi-arrow-left"></VIcon>
                                            Back
                                        </LinkButton>
